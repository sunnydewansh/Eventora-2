...
From the project root, install both apps and start them together:
...
Or use one command to install everything and start both apps:

```bash
npm run dev:all
```

The backend runs on the port configured in `server/.env`, and the frontend runs on the Vite URL shown in the terminal.
